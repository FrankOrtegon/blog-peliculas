...
test('test add publication', async () => {
  // given
  const persistedPublication = new Publication(123, 'El conjuro', 'Terror', 'Expedientes Warren', 'image');
  mockPublicationRepository.save = jest.fn(() => persistedPublication);

  // when
  const publica = await addPublication('El conjuro', 'Terror', 'Expedientes Warren', 'image', mockPublicationRepository );

  // then
  expect(mockPublicationRepository.save).toHaveBeenCalledWith(new Publication (null, 'El conjuro', 'Terror', 'Expedientes Warren', 'image'));
